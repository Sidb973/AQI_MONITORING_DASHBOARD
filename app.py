...
#############################################
# DOWNLOAD CENTER (Add this to the end of app.py)
#############################################
st.markdown("---")
st.header("💾 Download Center")
st.caption("Download the cleaned datasets and computed metrics generated by this dashboard.")

# 1. Helper function to convert DataFrame to CSV
@st.cache_data
def convert_df(df):
    # Cache the conversion to prevent re-encoding on every rerun
    return df.to_csv(index=False).encode('utf-8')

# 2. Gather all datasets you want to export
# We re-call the cached functions to ensure we get the DataFrames 
# regardless of which tab the user has visited.
clean_city_day = data["city_day"]
clean_station_day = data["station_day"]
city_kpis_export = compute_city_kpis(clean_city_day)
station_health_export = compute_station_health(data["raw_station_day"])
station_markers_export = classify_station_markers(clean_station_day)

# 3. Create columns for the download buttons
d_col1, d_col2, d_col3, d_col4, d_col5 = st.columns(5)

with d_col1:
    csv_city = convert_df(clean_city_day)
    st.download_button(
        label="📥 Cleaned City Data",
        data=csv_city,
        file_name='cleaned_city_day.csv',
        mime='text/csv',
    )

with d_col2:
    csv_station = convert_df(clean_station_day)
    st.download_button(
        label="📥 Cleaned Station Data",
        data=csv_station,
        file_name='cleaned_station_day.csv',
        mime='text/csv',
    )

with d_col3:
    csv_kpis = convert_df(city_kpis_export)
    st.download_button(
        label="📥 City Vulnerability KPIs",
        data=csv_kpis,
        file_name='city_vulnerability_kpis.csv',
        mime='text/csv',
    )

with d_col4:
    csv_health = convert_df(station_health_export)
    st.download_button(
        label="📥 Station Health Report",
        data=csv_health,
        file_name='station_health_metrics.csv',
        mime='text/csv',
    )

with d_col5:
    csv_markers = convert_df(station_markers_export)
    st.download_button(
        label="📥 Station Classifications",
        data=csv_markers,
        file_name='station_markers_industrial_vehicular.csv',
        mime='text/csv',
    )
